...
function newReq(func) {
    ...
    return function(userContext) {
        cli.debug("configuration passed to SDK: %j", userContext.sdk || { });
        sdk.utils.setup(userContext.sdk || { });
        // we need to camelcase the keys
        ...
        cli.debug("parameters passed to SDK module function: %j", params);
    ...
    self.run = function() {
        ...
        for (var flag in this) {
            ...
            if (self.commands[name]) {
                cli.debug("captured command: %s", name);
                fn = self.commands[name];
                break;
            }
        }
        if (self.commands.get && !fn) {
            cli.debug("no command specifed: defaulting to a get request");
